...
int main (int argc, char **argv)
{
  ...
  for (int y = 0; y < dst.height; y++)
  {
    for (int x = 0; x < dst.width; x++)
    {
      ...
      // On écrase au même emplacement mémoire les valeurs de gris
      pixel[0] = gray;
      pixel[1] = gray;
      pixel[2] = gray;
    }
  ...
  /**************************/
  /* TODO : Collecte des sous-parties dst dans l'emplacement src de root=0 */
  /**************************/
  MPI_Gather(
      dst.buf, dst.sz_in_bytes, MPI_BYTE,
      src.buf, dst.sz_in_bytes, MPI_BYTE, // ces arguments sont ignorés pour tout processus !=0 (ie !=root)
  ...
  /* End */
  /*******/
  // TODO : Sauvegarde de l'image modifiée src par le processus 0 (root)
  if (mpirank == 0) 
  {
    write_image_into_file(&src, PathDest, "");
  }
