                     ...
                     char **msg_rcv, int *taille_msg_rcv)
{
  ...
  int nbVoisins = graphe->nb_voisins[rank];
  int nbAllRequests = nbVoisins * 2;
  MPI_Request rqs[nbAllRequests];

  for (int iv = 0; iv < nbVoisins; iv++)
  {
    int rank_v = graphe->voisins[rank][iv];
    MPI_Isend(msg_snd[iv], taille_msg_snd[iv], MPI_CHAR, rank_v, 1000, MPI_COMM_WORLD, &(rqs[iv * 2]));
    MPI_Irecv(msg_rcv[iv], taille_msg_rcv[iv], MPI_CHAR, rank_v, 1000, MPI_COMM_WORLD, &(rqs[iv * 2 + 1]));
  }

  MPI_Waitall(nbAllRequests, rqs, MPI_STATUSES_IGNORE);
}
